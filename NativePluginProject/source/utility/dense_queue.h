 ...
 * 
 * It's your responsibility to understand and use the containers in a correct and safe way. Nothing
 * is hidden, there are no private members, everything is open. So, if you want to get funky with
 * memory there is nothing stopping you. It's assumed you will understand how the memory is managed
 * under the hood so you don't invalide the state of the containers.
 */
 ...
 * popping from both ends of the queue. For queue and stack semantics push, pop_fifo, and pop_lifo
 * helper methods remove any ambiguity with the use of push_front/push_back and pop_front/
 * pop_back.
 * 
...
struct DenseQueue {
    ...
    /**
     * @param _elementSizeB size in bytes of each item
     * @param _capacity     number of items in the buffer
     * @param buffer        pass in buffer to be used by DenseQueue with adequate size to hold
     *  (_elementSizeB * _capacity) bytes, or nullptr for DenseQueue to Q_malloc the buffer
     *  (asserts if ALLOW_MALLOC is not 1)
     * @param _assertOnFull	assert when a write fails due to reaching capacity, has no effect when
     *  SLOWCHECKS is not defined, on by default but some usages of DenseQueue may respond
     *  gracefully to a write failure, and therefore would not want to assert.
     */
    explicit DenseQueue(
        u16 _elementSizeB,
        u32 _capacity,
    ...
    /**
     * @returns pointer to the front item, or nullptr if empty
     */
    inline void* front() {
    ...
    /**
     * @returns pointer to the back item, or nullptr if empty
     */
    inline void* back() {
    ...
    /**
     * @returns pointer to the next open item at the back (back+1), or nullptr if full
     */
    ...
    /**
     * Use push, pop_fifo and pop_lifo for worry-free queue or stack semantics.
     */
    ...
    if (length + n <= capacity) {
        ...
        if (vals) {
            ...
            if (firstn < n) {
                memcpy(
                    items,
                    (void*)((uintptr_t)vals + (elementSizeB * firstn)),
                    elementSizeB * (n - firstn));
            }
        }
        else if (zero) {
...
u32 DenseQueue::pop_front_n(u32 n, void* dst)
{
    ...
    if (addr) {
        ...
        if (dst) {
            ...
            if (firstn < n) {
                memcpy(
                    (void*)((uintptr_t)dst + (elementSizeB * firstn)),
                    items,
                    elementSizeB * (n - firstn));
            }
        }
...
#define DenseQueue_Typed(Type, Name) \
    struct Name {\
...
#define DenseQueue_Typed_WithBuffer(Type, Name, _capacity, _assertOnFull) \
    struct Name {\
        enum { TypeSize = sizeof(Type) };\
        DenseQueue _q;\
